...
import { changeForSelectArray, getWordCount, parseError } from "@/lib/utils";
...
const AbstractSubmitForm = ({
  ...
  const onsubmit = async (data: z.output<typeof AbstractFormSchema>) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (key === "co_authors") {
        const coAuthorsWithOrder = value.map((author, index) => ({
          ...author,
          display_order: index + 1,
        }));

        formData.append("co_authors", JSON.stringify(coAuthorsWithOrder));
