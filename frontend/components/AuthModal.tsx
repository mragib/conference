...
export default function AuthModal({ initialView = "login" }) {
  ...
  /**
   * 🚀 UPDATED handleClose:
   * Explicitly pushes to home to clear the intercepted/parallel slot.
   * This prevents 'router.back()' from taking you to /legal if you
   * opened the modal from or within the legal context.
   */
  const handleClose = () => {
  ...
  const handleViewChange = (newView) => {
    ...
    if (newView === "login") router.replace("/login", { scroll: false });
    if (newView === "signup") router.replace("/signUp", { scroll: false });
  };
        ...
        <div className="flex flex-col md:flex-row flex-grow relative min-h-0 overflow-hidden">
          ...
          >
            ...
            <p className="text-[9px] text-white/40 italic font-black mt-12 uppercase tracking-widest">
              © International Conference on Building Resilient Supply Chains
              2026
            </p>
          </div>
